...
function ocn_run_with_ad(config_fp, k, backend)
    ...
    # Bundle the differentiated state into a single OceanModel so Checkpointing.jl
    # can snapshot it. The shadow zeroes Prog/Diag/Tend/dt and aliases the (inactive)
    # Mesh.
    ...
    # Checkpointed adjoint (Revolve): differentiates one timestep at a time so
    # Enzyme's per-step CUDA malloc-heap tape is freed between steps, instead of one
    # autodiff over the whole loop (which overflows the heap past ~4 steps).
    ocn_run_loop_checkpointed!(model, d_model, nsteps; nsnaps=4, verbose=1)

...
# size) on the CPU and the GPU. The checkpointed reverse-mode AD driver is timed
# by the companion script resolution_scaling_ad_benchmark.jl (same ladders, same
# harness); the two were split so a forward-only sweep needn't load Enzyme /
# Checkpointing and so each can be run and tuned independently.
#

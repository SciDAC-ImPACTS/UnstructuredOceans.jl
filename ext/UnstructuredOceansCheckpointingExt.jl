module UnstructuredOceansCheckpointingExt

# Checkpointed reverse-mode AD of UnstructuredOceans's forward model. This extension loads only
...
# types) live in UnstructuredOceansEnzymeExt, which is already loaded whenever Enzyme is, so they
...
import UnstructuredOceans: OceanModel
...
#
# The loss is sumGPU[1] = Σ ssh[end]², accumulated by UnstructuredOceans.sum_array after the loop.
...
function UnstructuredOceans.ocn_loss(model::OceanModel, sumGPU, scheme::Checkpointing.Scheme, nsteps::Int)
    @ad_checkpoint scheme for i = 1:nsteps
        UnstructuredOceans.ocn_step!(model)
    end
    backend = KA.get_backend(model.Prog.ssh[end])
    n = length(model.Prog.ssh[end])
    sumKernel! = UnstructuredOceans.sum_array(backend, 1)
...
function UnstructuredOceans.ocn_run_loop_checkpointed!(model::OceanModel, d_model::OceanModel,
    ...
    autodiff(mode, UnstructuredOceans.ocn_loss, Const,
...
end # module UnstructuredOceansCheckpointingExt
